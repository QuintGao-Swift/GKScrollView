...
class GKRotationManager_iOS_9_15: GKRotationManager {
    ...
    func rotationEnd() {
        if window?.isHidden == false && !UIInterfaceOrientationIsLandscape(currentOrientation) {
    ...
    func allowsRotation() -> Bool {
        if UIDeviceOrientationIsValidInterfaceOrientation(UIDevice.current.orientation) {

...
class VideoPlayerStatusBar: UIView {
    ...
    @objc private func networkDidChange(_ notify: Notification) {
        ...
        guard let status = userInfo["status"] as? NetworkReachabilityManager.NetworkReachabilityStatus else { return }
        networkLabel.text = NetworkReachabilityManager.networkType(status: status)
        setNeedsLayout()
        layoutIfNeeded()
    }

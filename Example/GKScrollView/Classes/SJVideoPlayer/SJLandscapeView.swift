...
class SJLandscapeView: VideoLandscapeView, SJControlLayer {
    ...
    override func backAction() {
        if let rotationManager = self.rotationManager {
    ...
    func installedControlView(to videoPlayer: SJBaseVideoPlayer!) {
        ...
        self.playBtn.isSelected = player?.isPlaying == true
        ...
        self.player?.gestureController.singleTapHandler = { [weak self] control, location in
            ...
            if self.isContainerShow {
                self.hideContainerView(animated: true)
            }else {
                self.showContainerView(animated: true)
